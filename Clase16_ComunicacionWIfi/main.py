...
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)#ndica que el socket utilizará el protocolo de Internet (IPv4), y socket.SOCK_STREAM indica que será un socket de tipo TCP.
s.bind(('', 80))
s.listen(5)

while True:
    try:
        conn, addr = s.accept()#Este método acepta una conexión entrante y devuelve dos valores: conn, que es un nuevo objeto de socket que representa la conexión, y addr, que es la dirección IP y el número de puerto del cliente que se ha conectado.
        print('Got a connection from %s' % str(addr))
        request = conn.recv(1024)#recibe datos con unt  tamaño maximo de 8byttes
        request = str(request)   
